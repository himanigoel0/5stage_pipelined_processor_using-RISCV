`timescale 1ns / 1ps
    ...
    module pipeline(
        ...
        assign jump_target = idex_pc + idex_imm;
...
//        wire idex_jump;
        ...
        assign pc_plus4_ex = idex_pc + 32'd4;
...
//        wire [31:0] exmem_imm;
        ...
        assign alu_src1 = (idex_opcode == 7'b0010111) ? idex_pc :   // AUIPC case
                          (forwardA == 2'b10) ? exmem_alu_result :  // EX/MEM forwarding case
                          (forwardA == 2'b01) ? wb_data :           // WB forwarding case
                          idex_rd1;                                 // else / normal case
...
//                                     alu_input2;
        ...
        assign uses_imm =
               ...
               (idex_opcode == 7'b0000011) || // LW
               (idex_opcode == 7'b0100011) || // SW
               (idex_opcode == 7'b0110111) || // LUI
               (idex_opcode == 7'b0010111) || // AUIPC
...
//                                     idex_rd2;
        ...
        // for the LUI and AUIPC case, imm are by default handled using the uses_imm logic
...
//        assign branch_taken = idex_branch && (alu_src1 == alu_src2_forwarded);
        assign flush = branch_taken || idex_jump;
        ...
        assign wb_data = memwb_jump ? memwb_pc_plus4 : memwb_memread ? memwb_mem_data : memwb_alu_result;
        ...
        always @(*) halt_debug = halt;
        
        always @(posedge clk or posedge rst) begin
            if(rst) halt <= 1'b0;
            else if(ecall) halt <= 1'b1; 
        end
        // we hold the ecall value, so that it doesnt change with the next instruction decoded at the next clk edge.
        // this is basically register holding previous value
